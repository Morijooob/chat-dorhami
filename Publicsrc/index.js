...
export class ChatRoom extends DurableObject {
  ...
  isMember(roomId,userId) { if(roomId==="global") return true; return this.ctx.storage.sql.exec("SELECT 1 FROM room_members WHERE room_id=? AND user_id=? LIMIT 1", roomId,userId).toArray().length>0; }
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/rooms") {
      ...
      const rooms=this.ctx.storage.sql.exec(`SELECT r.id,r.name,r.description,r.created_at,CASE WHEN r.id='global' THEN 1 ELSE 0 END is_public FROM rooms r WHERE r.id='global' OR EXISTS (SELECT 1 FROM room_members rm WHERE rm.room_id=r.id AND rm.user_id=?) ORDER BY CASE WHEN r.id='global' THEN 0 ELSE 1 END,r.created_at DESC`,userId).toArray();
      return json({rooms});
    }
    if (url.pathname === "/create-room" && request.method === "POST") {
      ...
      const creator=this.user(creatorId);
      if (!creator) return json({ok:false,error:"کاربر معتبر نیست."},401);
      if (!creator.vip) return json({ok:false,error:"ساخت اتاق خصوصی فقط برای اعضای VIP فعال است ⭐"},403);
      ...
      const room={id:id(),name,description:description||"اتاق خصوصی دورهمی",created_at:Date.now()};
      this.ctx.storage.sql.exec("INSERT INTO rooms (id,name,description,created_at) VALUES (?,?,?,?)",room.id,room.name,room.description,room.created_at);
      this.ctx.storage.sql.exec("INSERT INTO room_members (room_id,user_id,created_at) VALUES (?,?,?)",room.id,creatorId,Date.now());
      return json({ok:true,room},201);
    }
    if (url.pathname === "/room-invite" && request.method === "POST") {
      let data; try { data=await request.json(); } catch { return json({ok:false,error:"اطلاعات دعوت نامعتبر است."},400); }
      const roomId=clean(data.roomId,100), inviterId=clean(data.inviterId,100), target=normalizeUsername(data.username||data.userId);
      const room=this.room(roomId), inviter=this.user(inviterId);
      if(!room || room.id==="global") return json({ok:false,error:"این اتاق قابل مدیریت نیست."},400);
      if(!inviter || !inviter.vip) return json({ok:false,error:"مدیریت اتاق خصوصی فقط برای VIP است ⭐"},403);
      const creator=this.ctx.storage.sql.exec("SELECT user_id FROM room_members WHERE room_id=? ORDER BY created_at LIMIT 1",roomId).toArray()[0];
      if(!creator || creator.user_id!==inviterId) return json({ok:false,error:"فقط سازنده اتاق می‌تواند عضو اضافه کند."},403);
      const targetUser=this.ctx.storage.sql.exec("SELECT id,username,avatar,vip FROM users WHERE username=? OR id=? LIMIT 1",target,target).toArray()[0];
      if(!targetUser) return json({ok:false,error:"کاربر پیدا نشد."},404);
      this.ctx.storage.sql.exec("INSERT OR IGNORE INTO room_members (room_id,user_id,created_at) VALUES (?,?,?)",roomId,targetUser.id,Date.now());
      return json({ok:true,message:"کاربر با موفقیت به اتاق اضافه شد.",user:targetUser});
    }
    if (url.pathname === "/room-members" && request.method === "GET") {
      const roomId=clean(url.searchParams.get("roomId"),100), userId=clean(url.searchParams.get("userId"),100);
      if(!this.room(roomId))return json({ok:false,error:"اتاق پیدا نشد."},404);
      if(!this.isMember(roomId,userId))return json({ok:false,error:"شما عضو این اتاق نیستید."},403);
      const members=this.ctx.storage.sql.exec("SELECT u.id,u.username,u.avatar,u.vip FROM room_members rm JOIN users u ON u.id=rm.user_id WHERE rm.room_id=? ORDER BY rm.created_at",roomId).toArray();
      return json({members});
    }
    if (url.pathname === "/room-messages") { const roomId=clean(url.searchParams.get("roomId"),100)||"global", userId=clean(url.searchParams.get("userId"),100); if(!this.room(roomId))return json({ok:false,error:"اتاق پیدا نشد."},404); if(!this.isMember(roomId,userId))return json({ok:false,error:"شما عضو این اتاق نیستید."},403); return json({messages:this.ctx.storage.sql.exec(`${this.msgSelect()} WHERE m.room_id=? ORDER BY m.created_at DESC LIMIT 100`,roomId).toArray().reverse()}); }
    ...
    if (url.pathname === "/ws") {
      ...
      if(!this.room(roomId)) return json({ok:false,error:"اتاق پیدا نشد."},404); if(!this.isMember(roomId,userId)) return json({ok:false,error:"شما عضو این اتاق نیستید."},403);
  ...
  async webSocketMessage(ws,message){
    ...
    if(!this.isMember(roomId,userId)){ws.send(JSON.stringify({type:"error",message:"شما عضو این اتاق نیستید."}));try{ws.close(1008,"not a member")}catch{}return;}
    if(data.type==="voice_message"){
      ...
      if(!isPrivate && !this.isMember(targetRoom,userId)){ws.send(JSON.stringify({type:"error",message:"شما عضو این اتاق نیستید."}));return;}
