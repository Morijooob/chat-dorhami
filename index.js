...
export class ChatRoom{
 ...
 async fetch(r){await this.ready;let u=new URL(r.url);
  ...
  if((u.pathname==="/register"||u.pathname==="/login")&&r.method==="POST"){
   let b=await r.json().catch(()=>null);if(!b)return j({ok:false,error:"درخواست نامعتبر است"},400);let n=String(b.username||"").trim(),p=String(b.passwordHash||"");
   if(!/^[\\p{L}\\p{N}_]{3,24}$/u.test(n)||!/^[a-f0-9]{64}$/i.test(p))return j({ok:false,error:"نام کاربری یا رمز عبور نامعتبر است"},400);
   if(u.pathname==="/register"){let x=[...this.s.storage.sql.exec("SELECT id FROM users WHERE username=?",n)];if(x.length)return j({ok:false,error:"این نام کاربری قبلاً ثبت شده است"},409);let q=this.s.storage.sql.exec("INSERT INTO users(username,password_hash) VALUES(?,?)",n,p);return j({ok:true,user:{id:Number(q.lastInsertRowId),username:n}},201)}
  ...
  if(u.pathname==="/messages"&&r.method==="POST"){let b=await r.json().catch(()=>null),n=String(b?.username||"").trim(),t=String(b?.text||"").trim();if(!n||!t||t.length>1000)return j({ok:false,error:"پیام نامعتبر است"},400);this.s.storage.sql.exec("INSERT INTO messages(username,text) VALUES(?,?)",n,t);return j({ok:true})}
