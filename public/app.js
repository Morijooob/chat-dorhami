...
function addMessage(m){if(!m)return;const box=$('messages'),mine=me&&m.user_id?Number(m.user_id)===Number(me.id):me&&m.username===me.username,el=document.createElement('div');el.className='bubble '+(mine?'mine':'');el.innerHTML=`<div class="bubble-head"><span class="message-avatar">${getAvatar(m.user_id)}</span><span class="name">${esc(m.username)}</span></div><div>${esc(m.body)}</div><time>${formatTime(m.created_at)}</time>`;box.appendChild(el);box.scrollTop=box.scrollHeight;}
