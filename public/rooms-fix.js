(()=>{
  ...
  async function consumePendingJoin(){
    if(joining||!isChatVisible())return;let code='';try{code=new URLSearchParams(location.search).get('join')||''}catch(e){}code=(code||localStorage.getItem('dorhami_pending_room')||'').trim().toUpperCase();if(!code)return;joining=true;localStorage.removeItem('dorhami_pending_room');
