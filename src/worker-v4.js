...
export class Dorhami extends DurableObject {
  ...
  diagnostic(){try{const tables=this.ctx.storage.sql.exec("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").toArray().map(x=>x.name);return json({ok:true,version:'4.1',tables,counts:{users:this.countRows('users'),sessions:this.countRows('sessions'),messages:this.countRows('messages')}});}catch(error){console.error('DORHAMI_DIAGNOSTIC',error?.stack||error);return json({ok:false,code:'DIAGNOSTIC_FAILED'},500);}}
  countRows(table){if(!['users','sessions','messages'].includes(table))throw new Error('INVALID_TABLE');return this.ctx.storage.sql.exec(`SELECT COUNT(*) AS count FROM ${table}`).toArray()[0]?.count??0;}
  ...
  readHistory(){return this.ctx.storage.sql.exec(`SELECT id,username,body,created_at FROM messages WHERE room=? ORDER BY id DESC LIMIT 100`,ROOM).toArray().reverse();}
  privateRoom(a,b){const x=Number(a),y=Number(b);return x<y?`private:${x}:${y}`:`private:${y}:${x}`;}
  privateHistory(a,b){const room=this.privateRoom(a,b);return this.ctx.storage.sql.exec(`SELECT id,user_id,username,body,created_at FROM messages WHERE room=? ORDER BY id DESC LIMIT 100`,room).toArray().reverse();}
