...
export class Dorhami extends DurableObject {
  ...
  async fetch(request) {
    ...
    try {
      ...
      console.error('DORHAMI_ERROR', requestId, error?.stack || error);
      return json({ error: 'خطای داخلی سرور', code: 'SERVER_ERROR', requestId }, 500);
    }
  }
  ...
  health() {
    try {
      ...
      return json({ ok: true, version: '4.1', storage: 'sqlite', websocket: 'hibernation', pbkdf2Iterations: PBKDF2_ITERATIONS });
    } catch (error) {
      console.error('DORHAMI_HEALTH', error?.stack || error);
      return json({ ok: false, code: 'STORAGE_UNAVAILABLE' }, 500);
    }
  }

  diagnostic() {
    try {
      const tables = this.ctx.storage.sql.exec("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").toArray().map(x => x.name);
      return json({
        ok: true,
        version: '4.1',
        tables,
        counts: {
          ...
          sessions: this.countRows('sessions'),
  ...
  countRows(table) {
    if (!['users', 'sessions', 'messages'].includes(table)) throw new Error('INVALID_TABLE');
    return this.ctx.storage.sql.exec(`SELECT COUNT(*) AS count FROM ${table}`).toArray()[0]?.count ?? 0;
  }
  ...
  async register(request) {
    ...
    try {
      this.ctx.storage.sql.exec(
        ...
        username, passwordHash, b64(salt), Date.now()
      );
    } catch (error) {
      const existing = this.ctx.storage.sql.exec('SELECT id FROM users WHERE username=? LIMIT 1', username).toArray()[0];
      if (existing) return json({ error: 'این نام کاربری قبلاً ثبت شده است.', code: 'USERNAME_TAKEN' }, 409);
    ...
    const user = this.ctx.storage.sql.exec('SELECT id,username FROM users WHERE username=? LIMIT 1', username).toArray()[0];
    if (!user) return json({ error: 'حساب ساخته نشد.', code: 'REGISTER_VERIFY_FAILED' }, 500);
  ...
  async login(request) {
    ...
    const username = cleanName(data.username);
    const password = String(data.password ?? '');
    if (!username || !password) return json({ error: 'نام کاربری و رمز عبور را وارد کن.', code: 'MISSING_CREDENTIALS' }, 400);

    const user = this.ctx.storage.sql.exec(
      'SELECT id,username,password_hash,salt FROM users WHERE username=? LIMIT 1', username
    ).toArray()[0];
    if (!user) return json({ error: 'نام کاربری یا رمز عبور اشتباه است.', code: 'INVALID_CREDENTIALS' }, 401);

    try {
      const computed = await hashPassword(password, fromB64(user.salt));
      if (!safeEqual(user.password_hash, computed)) {
    ...
    try {
      return withCookie(json({ ok: true, user: { id: user.id, username: user.username } }), await this.createSession(user.id));
    } catch (error) {
  ...
  async createSession(userId) {
    ...
    this.ctx.storage.sql.exec(
      ...
      tokenHash, userId, Date.now() + SESSION_MS
    );
    return token;
  }
  ...
  async auth(request) {
    ...
    const row = this.ctx.storage.sql.exec(
      `SELECT u.id,u.username FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>? LIMIT 1`,
  ...
  async logout(request) {
    const token = getCookie(request, COOKIE);
    if (token) this.ctx.storage.sql.exec('DELETE FROM sessions WHERE token_hash=?', await sha256(token));
  ...
  async rooms(request) {
    ...
    return json({ rooms: [{ id: ROOM, name: 'دورهمی عمومی', online: this.connectedUsers().length }], messages: this.readHistory() });
  }

  async websocket(request) {
    if (request.headers.get('Upgrade')?.toLowerCase() !== 'websocket') return json({ error: 'WebSocket required', code: 'WEBSOCKET_REQUIRED' }, 426);
    const user = await this.auth(request);
    if (!user) return json({ error: 'ابتدا وارد حساب شو.', code: 'AUTH_REQUIRED' }, 401);
  ...
  async webSocketMessage(ws, message) {
    try {
      ...
      if (!data) return;
      if (data.type === 'history') return ws.send(JSON.stringify({ type: 'history', messages: this.readHistory() }));
      if (data.type === 'message') return this.sendMessage(user, data.body);
  ...
  readHistory() {
    return this.ctx.storage.sql.exec(
      `SELECT id,username,body,created_at FROM messages WHERE room=? ORDER BY id DESC LIMIT 100`, ROOM
    ).toArray().reverse();
  }
  ...
  async sendMessage(user, rawBody) {
    ...
    this.ctx.storage.sql.exec(
      ...
      ROOM, user.id, user.username, body, now
    );
    const message = this.ctx.storage.sql.exec(
  ...
  broadcast(data) {
    ...
    for (const ws of this.ctx.getWebSockets()) {
      try { if (ws.readyState === WebSocket.OPEN) ws.send(text); } catch (error) { console.error('DORHAMI_BROADCAST', error); }
    }
  }
  ...
  connectedUsers() {
    const users = [];
    for (const ws of this.ctx.getWebSockets()) {
      try { const user = this.userFromConnection(ws.deserializeAttachment()); if (user) users.push(user); } catch {}
    }
    return users;
  }
...
// Preserve the legacy Durable Object export required by the existing namespace.
...
async function readJson(request) {
  try { const data = await request.json(); return data && typeof data === 'object' ? data : {}; }
  catch { return {}; }
}
...
function withCookie(response, token) {
  const headers = new Headers(response.headers);
  headers.set('Set-Cookie', `${COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${SESSION_MS / 1000}`);
  return new Response(response.body, { status: response.status, headers });
}
