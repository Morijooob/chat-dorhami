...
export class Dorhami extends DurableObject {
  ...
  diagnostic(){try{const tables=this.ctx.storage.sql.exec("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").toArray().map(x=>x.name);return json({ok:true,version:'4.1',tables,counts:{users:this.countRows('users'),sessions:this.countRows('sessions'),messages:this.countRows('messages'),reactions:this.countRows('reactions'),vip_memberships:this.countRows('vip_memberships')}});}catch(error){console.error('DORHAMI_DIAGNOSTIC',error?.stack||error);return json({ok:false,code:'DIAGNOSTIC_FAILED'},500);}}
  countRows(table){if(!['users','sessions','messages','reactions','vip_memberships'].includes(table))throw new Error('INVALID_TABLE');return this.ctx.storage.sql.exec(`SELECT COUNT(*) AS count FROM ${table}`).toArray()[0]?.count??0;}
