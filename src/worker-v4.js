...
const ADMIN_COOKIE = 'dorhami_admin_session';
const ROOM = 'general';
...
const ADMIN_BOOTSTRAP_SALT = '80RuMh/KWlOKqegxYeP3hw==';
const ADMIN_BOOTSTRAP_HASH = '44kSnjjI8xYdTQVBYXGE79Ntift2/u0ApmkKHsQANv0=';

export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === '/admin') return env.ASSETS.fetch(new Request(new URL('/admin.html', request.url), request));
...
export class Dorhami extends DurableObject {
  ...
  ensureSchema() {
    ...
    sql.exec(`CREATE TABLE IF NOT EXISTS vip_memberships (user_id INTEGER PRIMARY KEY, vip_until INTEGER NOT NULL DEFAULT 0)`);
    sql.exec('CREATE INDEX IF NOT EXISTS vip_memberships_vip_until ON vip_memberships(vip_until)');
    sql.exec(`CREATE TABLE IF NOT EXISTS admin_accounts (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE COLLATE NOCASE, password_hash TEXT NOT NULL, salt TEXT NOT NULL, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)`);
    sql.exec(`CREATE TABLE IF NOT EXISTS admin_sessions (token_hash TEXT PRIMARY KEY, admin_id INTEGER NOT NULL, expires_at INTEGER NOT NULL)`);
    sql.exec('CREATE INDEX IF NOT EXISTS admin_sessions_admin_id ON admin_sessions(admin_id)');
    const admin=sql.exec('SELECT id FROM admin_accounts WHERE username=? LIMIT 1','admin').toArray()[0];
    if(!admin) sql.exec('INSERT INTO admin_accounts(username,password_hash,salt,created_at,updated_at) VALUES(?,?,?,?,?)','admin',ADMIN_BOOTSTRAP_HASH,ADMIN_BOOTSTRAP_SALT,Date.now(),Date.now());
  }
  async fetch(request) {
    const url=new URL(request.url);
    try {
      ...
      if(url.pathname==='/api/vip/status') return this.vipStatus(request);
      if(url.pathname==='/api/rooms') return this.rooms(request);
      if(url.pathname==='/api/admin/login') return request.method==='POST'?this.adminLogin(request):methodNotAllowed();
      if(url.pathname==='/api/admin/logout') return request.method==='POST'?this.adminLogout(request):methodNotAllowed();
      if(url.pathname==='/api/admin/me') return this.adminMe(request);
      if(url.pathname==='/api/admin/dashboard') return this.adminDashboard(request);
      if(url.pathname==='/api/admin/password') return request.method==='POST'?this.adminChangePassword(request):methodNotAllowed();
  ...
  health(){try{this.ctx.storage.sql.exec('SELECT 1').toArray();return json({ok:true,version:'4.2',storage:'sqlite',websocket:'hibernation',pbkdf2Iterations:PBKDF2_ITERATIONS});}catch(error){console.error('DORHAMI_HEALTH',error?.stack||error);return json({ok:false,code:'STORAGE_UNAVAILABLE'},500);}}
  diagnostic(){try{const tables=this.ctx.storage.sql.exec("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").toArray().map(x=>x.name);return json({ok:true,version:'4.2',tables,counts:{users:this.countRows('users'),sessions:this.countRows('sessions'),messages:this.countRows('messages'),reactions:this.countRows('reactions'),vip_memberships:this.countRows('vip_memberships'),admin_accounts:this.countRows('admin_accounts'),admin_sessions:this.countRows('admin_sessions')}});}catch(error){console.error('DORHAMI_DIAGNOSTIC',error?.stack||error);return json({ok:false,code:'DIAGNOSTIC_FAILED'},500);}}
  countRows(table){if(!['users','sessions','messages','reactions','vip_memberships','admin_accounts','admin_sessions'].includes(table))throw new Error('INVALID_TABLE');return this.ctx.storage.sql.exec(`SELECT COUNT(*) AS count FROM ${table}`).toArray()[0]?.count??0;}
  ...
  async createAdminSession(adminId){const tokenBytes=crypto.getRandomValues(new Uint8Array(32)),token=b64(tokenBytes),tokenHash=await sha256(token);this.ctx.storage.sql.exec('INSERT INTO admin_sessions(token_hash,admin_id,expires_at) VALUES(?,?,?)',tokenHash,adminId,Date.now()+ADMIN_SESSION_MS);return token;}
  async adminAuth(request){const token=getCookie(request,ADMIN_COOKIE);if(!token)return null;const row=this.ctx.storage.sql.exec(`SELECT a.id,a.username FROM admin_sessions s JOIN admin_accounts a ON a.id=s.admin_id WHERE s.token_hash=? AND s.expires_at>? LIMIT 1`,await sha256(token),Date.now()).toArray()[0];return row||null;}
  async adminLogin(request){const data=await readJson(request),username=cleanName(data.username||'admin'),password=String(data.password??'');if(!username||!password)return json({error:'نام کاربری و رمز مدیریت را وارد کن.',code:'MISSING_CREDENTIALS'},400);const admin=this.ctx.storage.sql.exec('SELECT id,username,password_hash,salt FROM admin_accounts WHERE username=? LIMIT 1',username).toArray()[0];if(!admin)return json({error:'نام کاربری یا رمز مدیریت اشتباه است.',code:'INVALID_ADMIN_CREDENTIALS'},401);try{const computed=await hashPassword(password,fromB64(admin.salt));if(!safeEqual(admin.password_hash,computed))return json({error:'نام کاربری یا رمز مدیریت اشتباه است.',code:'INVALID_ADMIN_CREDENTIALS'},401);}catch(error){console.error('DORHAMI_ADMIN_LOGIN_HASH',error?.stack||error);return json({error:'خطا در بررسی رمز مدیریت.',code:'ADMIN_LOGIN_HASH_FAILED'},500);}const token=await this.createAdminSession(admin.id);return withAdminCookie(json({ok:true,admin:{id:admin.id,username:admin.username}}),token);}
  async adminLogout(request){const token=getCookie(request,ADMIN_COOKIE);if(token)this.ctx.storage.sql.exec('DELETE FROM admin_sessions WHERE token_hash=?',await sha256(token));const response=json({ok:true}),headers=new Headers(response.headers);headers.set('Set-Cookie',clearAdminCookie());return new Response(response.body,{status:response.status,headers});}
  async adminMe(request){const admin=await this.adminAuth(request);return json({authenticated:Boolean(admin),admin});}
  async requireAdmin(request){return this.adminAuth(request);}
  async adminDashboard(request){const admin=await this.requireAdmin(request);if(!admin)return json({error:'ابتدا وارد پنل مدیریت شو.',code:'ADMIN_AUTH_REQUIRED'},401);const users=this.ctx.storage.sql.exec('SELECT id,username,created_at FROM users ORDER BY id DESC LIMIT 100').toArray();const vip=this.ctx.storage.sql.exec('SELECT COUNT(*) AS count FROM vip_memberships WHERE vip_until>?',Date.now()).toArray()[0]?.count??0;return json({ok:true,admin,stats:{users:this.countRows('users'),online:this.connectedUsers().length,messages:this.countRows('messages'),vip:Number(vip)||0},users});}
  async adminChangePassword(request){const admin=await this.requireAdmin(request);if(!admin)return json({error:'ابتدا وارد پنل مدیریت شو.',code:'ADMIN_AUTH_REQUIRED'},401);const data=await readJson(request),current=String(data.currentPassword??''),next=String(data.newPassword??'');if(next.length<8)return json({error:'رمز جدید باید حداقل ۸ کاراکتر باشد.',code:'WEAK_PASSWORD'},400);const row=this.ctx.storage.sql.exec('SELECT id,password_hash,salt FROM admin_accounts WHERE id=? LIMIT 1',admin.id).toArray()[0];if(!row)return json({error:'حساب مدیریت پیدا نشد.',code:'ADMIN_NOT_FOUND'},404);const computed=await hashPassword(current,fromB64(row.salt));if(!safeEqual(row.password_hash,computed))return json({error:'رمز فعلی اشتباه است.',code:'CURRENT_PASSWORD_INVALID'},401);const salt=crypto.getRandomValues(new Uint8Array(16)),passwordHash=await hashPassword(next,salt),now=Date.now();this.ctx.storage.sql.exec('UPDATE admin_accounts SET password_hash=?,salt=?,updated_at=? WHERE id=?',passwordHash,b64(salt),now,admin.id);this.ctx.storage.sql.exec('DELETE FROM admin_sessions WHERE admin_id=?',admin.id);const token=await this.createAdminSession(admin.id);return withAdminCookie(json({ok:true,message:'رمز مدیریت با موفقیت تغییر کرد.'}),token);}
  isVip(userId){const row=this.ctx.storage.sql.exec('SELECT vip_until FROM vip_memberships WHERE user_id=? LIMIT 1',Number(userId)).toArray()[0];return Number(row?.vip_until||0)>Date.now();}
  vipInfo(userId){const row=this.ctx.storage.sql.exec('SELECT vip_until FROM vip_memberships WHERE user_id=? LIMIT 1',Number(userId)).toArray()[0];const vipUntil=Number(row?.vip_until||0);return {vip:vipUntil>Date.now(),vipUntil:vipUntil||null};}
  async me(request){try{const user=await this.auth(request);return json({authenticated:Boolean(user),user:user?{...user,...this.vipInfo(user.id)}:null});}catch(error){console.error('DORHAMI_ME',error?.stack||error);return json({error:'خطا در بررسی نشست.',code:'SESSION_CHECK_FAILED'},500);}}
  async vipStatus(request){try{const user=await this.auth(request);if(!user)return json({error:'ابتدا وارد حساب شو.',code:'AUTH_REQUIRED'},401);return json({ok:true,userId:user.id,...this.vipInfo(user.id)});}catch(error){console.error('DORHAMI_VIP_STATUS',error?.stack||error);return json({error:'خطا در بررسی وضعیت VIP.',code:'VIP_STATUS_FAILED'},500);}}
  ...
  async websocket(request){if(request.headers.get('Upgrade')?.toLowerCase()!=='websocket')return json({error:'WebSocket required',code:'WEBSOCKET_REQUIRED'},426);const user=await this.auth(request);if(!user)return json({error:'ابتدا وارد حساب شو.',code:'AUTH_REQUIRED'},401);const pair=new WebSocketPair(),client=pair[0],server=pair[1];this.ctx.acceptWebSocket(server);server.serializeAttachment({userId:user.id,username:user.username,room:ROOM});server.send(JSON.stringify({type:'ready',user:{...user,...this.vipInfo(user.id)},room:ROOM,messages:this.readHistory()}));this.broadcastPresence();return new Response(null,{status:101,webSocket:client});}
...
function withAdminCookie(response,token){const headers=new Headers(response.headers);headers.set('Set-Cookie',`${ADMIN_COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${ADMIN_SESSION_MS/1000}`);return new Response(response.body,{status:response.status,headers});}
function clearCookie(){return `${COOKIE}=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0`;}
function clearAdminCookie(){return `${ADMIN_COOKIE}=; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=0`;}
