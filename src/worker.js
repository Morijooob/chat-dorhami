...
export class Dorhami {
  ...
  async auth(request) {
    ...
    const tokenHash = await sha256(token);
    const row = this.state.storage.sql.exec(`SELECT u.id,u.username,u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>?`, tokenHash, Date.now()).one();
    return row || null;
  }
  ...
  async rooms(request) {
    ...
    const messages = this.state.storage.sql.exec(`SELECT id,username,body,created_at FROM messages WHERE room=? ORDER BY id DESC LIMIT 80`, ROOM).toArray().reverse();
  ...
  broadcastPresence() {
    this.broadcast({ type: 'presence', count: this.clients.size, users: [...this.clients.values()].map(x => ({ id: x.user.id, username: x.user.username, role: x.user.role })) });
  }
}
